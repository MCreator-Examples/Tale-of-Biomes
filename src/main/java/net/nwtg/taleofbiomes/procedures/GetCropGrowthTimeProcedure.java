...
public class GetCropGrowthTimeProcedure {
				...
				try {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("growth_time") && (fmObjCropsRoot.get("growth_time").isJsonPrimitive() ? fmObjCropsRoot.get("growth_time").getAsJsonPrimitive().isNumber() : false)) {
								nValue = fmObjCropsRoot.get("growth_time").getAsDouble();
							}
						}
					}
				} catch (IOException e) {
