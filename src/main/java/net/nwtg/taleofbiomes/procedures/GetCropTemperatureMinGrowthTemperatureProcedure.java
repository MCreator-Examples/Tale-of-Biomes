...
public class GetCropTemperatureMinGrowthTemperatureProcedure {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("temperature") && fmObjCropsRoot.get("temperature").isJsonObject()) {
								...
								if (fmObjCropsRootGroup.has("min_growth_temperature")
										&& (fmObjCropsRootGroup.get("min_growth_temperature").isJsonPrimitive() ? fmObjCropsRootGroup.get("min_growth_temperature").getAsJsonPrimitive().isNumber() : false)) {
									nValue = fmObjCropsRootGroup.get("min_growth_temperature").getAsDouble();
								}
							}
						}
					}
