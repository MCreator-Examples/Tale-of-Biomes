...
public class GetCropTemperatureMinCriticalTimeProcedure {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("temperature") && fmObjCropsRoot.get("temperature").isJsonObject()) {
								...
								if (fmObjCropsRootGroup.has("min_critical_time") && (fmObjCropsRootGroup.get("min_critical_time").isJsonPrimitive() ? fmObjCropsRootGroup.get("min_critical_time").getAsJsonPrimitive().isNumber() : false)) {
									nValue = fmObjCropsRootGroup.get("min_critical_time").getAsDouble();
								}
							}
						}
					}
