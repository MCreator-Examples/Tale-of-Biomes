...
public class GetCropLightMinLightLevelProcedure {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("light") && fmObjCropsRoot.get("light").isJsonObject()) {
								...
								if (fmObjCropsRootGroup.has("min_light_level") && (fmObjCropsRootGroup.get("min_light_level").isJsonPrimitive() ? fmObjCropsRootGroup.get("min_light_level").getAsJsonPrimitive().isNumber() : false)) {
									nValue = fmObjCropsRootGroup.get("min_light_level").getAsDouble();
								}
							}
						}
					}
