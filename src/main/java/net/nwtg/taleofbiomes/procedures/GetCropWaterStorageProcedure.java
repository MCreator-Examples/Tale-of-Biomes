...
public class GetCropWaterStorageProcedure {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("water") && fmObjCropsRoot.get("water").isJsonObject()) {
								...
								if (fmObjCropsRootGroup.has("storage") && (fmObjCropsRootGroup.get("storage").isJsonPrimitive() ? fmObjCropsRootGroup.get("storage").getAsJsonPrimitive().isNumber() : false)) {
									nValue = fmObjCropsRootGroup.get("storage").getAsDouble();
								}
							}
						}
					}
