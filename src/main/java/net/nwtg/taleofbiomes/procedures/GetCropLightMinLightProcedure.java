...
public class GetCropLightMinLightProcedure {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("light") && fmObjCropsRoot.get("light").isJsonObject()) {
								...
								if (fmObjCropsRootGroup.has("min_light") && (fmObjCropsRootGroup.get("min_light").isJsonPrimitive() ? fmObjCropsRootGroup.get("min_light").getAsJsonPrimitive().isNumber() : false)) {
									nValue = fmObjCropsRootGroup.get("min_light").getAsDouble();
								}
							}
						}
					}
