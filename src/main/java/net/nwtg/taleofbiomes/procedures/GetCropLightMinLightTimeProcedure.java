...
public class GetCropLightMinLightTimeProcedure {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("light") && fmObjCropsRoot.get("light").isJsonObject()) {
								...
								if (fmObjCropsRootGroup.has("min_light_time") && (fmObjCropsRootGroup.get("min_light_time").isJsonPrimitive() ? fmObjCropsRootGroup.get("min_light_time").getAsJsonPrimitive().isNumber() : false)) {
									nValue = fmObjCropsRootGroup.get("min_light_time").getAsDouble();
								}
							}
						}
					}
