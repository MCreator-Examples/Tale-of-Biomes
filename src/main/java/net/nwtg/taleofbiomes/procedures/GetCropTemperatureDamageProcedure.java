...
public class GetCropTemperatureDamageProcedure {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("temperature") && fmObjCropsRoot.get("temperature").isJsonObject()) {
								...
								if (fmObjCropsRootGroup.has("damage") && (fmObjCropsRootGroup.get("damage").isJsonPrimitive() ? fmObjCropsRootGroup.get("damage").getAsJsonPrimitive().isNumber() : false)) {
									nValue = fmObjCropsRootGroup.get("damage").getAsDouble();
								}
							}
						}
					}
