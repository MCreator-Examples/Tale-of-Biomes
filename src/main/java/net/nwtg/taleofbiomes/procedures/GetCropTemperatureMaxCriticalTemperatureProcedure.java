...
public class GetCropTemperatureMaxCriticalTemperatureProcedure {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("temperature") && fmObjCropsRoot.get("temperature").isJsonObject()) {
								...
								if (fmObjCropsRootGroup.has("max_critical_temperature")
										&& (fmObjCropsRootGroup.get("max_critical_temperature").isJsonPrimitive() ? fmObjCropsRootGroup.get("max_critical_temperature").getAsJsonPrimitive().isNumber() : false)) {
									nValue = fmObjCropsRootGroup.get("max_critical_temperature").getAsDouble();
								}
							}
						}
					}
