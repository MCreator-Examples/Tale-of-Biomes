...
public class GetCropMaxHealthProcedure {
				...
				try {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("max_health") && (fmObjCropsRoot.get("max_health").isJsonPrimitive() ? fmObjCropsRoot.get("max_health").getAsJsonPrimitive().isNumber() : false)) {
								nValue = fmObjCropsRoot.get("max_health").getAsDouble();
							}
						}
					}
				} catch (IOException e) {
