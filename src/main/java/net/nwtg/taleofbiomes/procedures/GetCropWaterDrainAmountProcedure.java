...
public class GetCropWaterDrainAmountProcedure {
					...
					if (fmObjRoot.has("crops") && fmObjRoot.get("crops").isJsonObject()) {
						...
						if (fmObjCrops.has(sBlockRegistry) && fmObjCrops.get(sBlockRegistry).isJsonObject()) {
							...
							if (fmObjCropsRoot.has("water") && fmObjCropsRoot.get("water").isJsonObject()) {
								...
								if (fmObjCropsRootGroup.has("drain_amount") && (fmObjCropsRootGroup.get("drain_amount").isJsonPrimitive() ? fmObjCropsRootGroup.get("drain_amount").getAsJsonPrimitive().isNumber() : false)) {
									nValue = fmObjCropsRootGroup.get("drain_amount").getAsDouble();
								}
							}
						}
					}
